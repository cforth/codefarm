...
class Exam
  ...
  def respond_to? (method)
    @data_source.respond_to?("get_#{method}_question") || super
  end

  def method_missing(name)
    super if !@data_source.respond_to?("get_#{name}_question")
...
puts exam.dxt
puts exam.respond_to?("dxt")
puts exam.respond_to?("dxt123")
