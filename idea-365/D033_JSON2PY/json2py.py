...
def write_python_file(file_path, string, mode='a'):
    with codecs.open(file_path, mode, "utf-8") as f:
        f.write(string)


def head_string(tkinter_class):
    string = "# This class automatically generated by json2py.py, do not manually modify!!!\n"
    string += "import tkinter as tk\n"
    string += "import tkinter.ttk as ttk\n\n\n"
    string += "class UI(%s.Frame):\n" % tkinter_class
...
def create_ui(json_file, output_file):
    ...
    write_python_file(output_file, head_string("ttk"), 'w')
    ...
    for k in widget:
        ...
        if widget_var:
            ...
            if widget_class == "Scale":
                write_python_file(output_file, "        self.%s = tk.DoubleVar()\n" % widget_var)
                write_python_file(output_file, "        self.%s['variable'] = self.%s\n" % (k, widget_var))
            else:
                write_python_file(output_file, "        self.%s = tk.StringVar()\n" % widget_var)
                write_python_file(output_file, "        self.%s['textvariable'] = self.%s\n" % (k, widget_var))
        # 使用grid布局控件
        ...
        if widget[k].get("bind"):
            for w in widget[k]["bind"]:
                write_python_file(output_file, "        self.%s.bind('%s', self.%s)\n" % (k, w, widget[k]["bind"][w]))
